    ...
    if step % 250 == 0:
        ...
        if step % 5000 == 0:
            print(f"  {step:6d}/{N_STEPS}  train={loss.item():.4f}  "
                  f"eval={ev:.4f}  best={best_loss:.4f}  {time.time()-t0:.0f}s",
                  flush=True)

torch.save(best_state, str(__import__("pathlib").Path(__file__).resolve().parent / "elya_n64.pt"))
...
out_path = str(__import__("pathlib").Path(__file__).resolve().parent / "sophia_weights_gen.bin")
buf = bytearray()
